...
from pyngsi.utils.mqttclient import MqttClient
...
def mock_broker(mocker):
    mocker.patch("paho.mqtt.client.Client.connect",
                 side_effect=lambda host, port, *_: logger.info(f"connect to MQTT broker at {host}:{port}"))
    mocker.patch("paho.mqtt.client.Client.username_pw_set",
                 side_effect=lambda user, passwd: logger.info(f"set MQTT credentials ({user}, {passwd})"))
