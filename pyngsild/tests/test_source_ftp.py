...
from pyngsild.source.sourceftp import SourceFtp
...
def mock_ftp(mocker):
    ...
    mocker.patch("ftplib.FTP.close", side_effect=lambda: logger.info("mock FTP close"))
...
def mock_ftpclient(mocker):
    mocker.patch(
        "pyngsild.ftpclient.FtpClient.retrieve_filelist",
        side_effect=mocked_retrieve_filelist,
    )
    mocker.patch("pyngsild.ftpclient.FtpClient.download", side_effect=mocked_download)
...
def test_retrieve_all_files(mock_ftp, mock_tempfile, mock_ftpclient):
    src = SourceFtp(
        "ftp.ncdc.noaa.gov", paths=["/pub/data/noaa/2019"], f_match=lambda x: True
    )
    assert len(src.downloaded_files) == 6
    assert (
        "/tmp/166240-99999-2019.gz",
        "/pub/data/noaa/2019/166240-99999-2019.gz",
    ) in src.downloaded_files
    assert (
        "/tmp/166270-99999-2019.gz",
        "/pub/data/noaa/2019/166270-99999-2019.gz",
    ) in src.downloaded_files
...
def test_retrieve_some_files(mock_ftp, mock_tempfile, mock_ftpclient):
    ...
    src = SourceFtp(
        "ftp.ncdc.noaa.gov", paths=["/pub/data/noaa"], f_match=lambda x: prog.match(x)
    )
    assert len(src.downloaded_files) == 2
    assert (
        "/tmp/166220-99999-2018.gz",
        "/pub/data/noaa/2018/166220-99999-2018.gz",
    ) in src.downloaded_files
    assert (
        "/tmp/166220-99999-2019.gz",
        "/pub/data/noaa/2019/166220-99999-2019.gz",
    ) in src.downloaded_files
